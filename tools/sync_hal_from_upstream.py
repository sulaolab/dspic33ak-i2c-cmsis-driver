#!/usr/bin/env python3
"""Synchronize the vendored dsPIC33AK I2C HAL from the upstream HAL repo."""
...
UPSTREAM_REPO = "https://github.com/sulaolab/dspic33ak-i2c-hal.git"
...
def update_upstream_md(repo_root: Path, upstream_commit: str) -> None:
    ...
    if replacements != 1:
        raise SystemExit("Could not update upstream commit line in src/hal_i2c/UPSTREAM.md")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="dspic33ak_i2c_hal_") as temp_dir:
    ...
    print(
        ...
        f"sulaolab/dspic33ak-i2c-hal @ {upstream_commit}"
    )
    return 0
